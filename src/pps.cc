...
bool pps_range_extension::read(bitreader* br, parse_context* ctx, const pic_parameter_set* pps)
{
  ...
  if (pps->transform_skip_enabled_flag) {
    ...
    if (uvlc == UVLC_ERROR ||
      ...
      //ctx->add_warning(P265_WARNING_PPS_HEADER_INVALID, false);
      //return false;
    }
  ...
  uvlc = get_uvlc(br);
  if (uvlc == UVLC_ERROR ||
      uvlc > libP265_max(0, sps->BitDepth_Y-10)) {
    ctx->add_warning(P265_WARNING_PPS_HEADER_INVALID, false);
    return false;
  }
  ...
  uvlc = get_uvlc(br);
  if (uvlc == UVLC_ERROR ||
      uvlc > libP265_max(0, sps->BitDepth_C-10)) {
    ctx->add_warning(P265_WARNING_PPS_HEADER_INVALID, false);
    return false;
  }
...
void pic_parameter_set::set_defaults(enum PresetSet)
{
  ...
  for (int i=0;i<P265_MAX_TILE_COLUMNS;i++) { colWidth[i]=0; }
  for (int i=0;i<P265_MAX_TILE_ROWS;i++)    { rowHeight[i]=0; }
  for (int i=0;i<=P265_MAX_TILE_COLUMNS;i++) { colBd[i]=0; }
  for (int i=0;i<=P265_MAX_TILE_ROWS;i++)    { rowBd[i]=0; }
...
bool pic_parameter_set::read(bitreader* br, parse_context* ctx)
{
  reset();
  ...
  if (uvlc >= P265_MAX_PPS_SETS ||
      uvlc == UVLC_ERROR) {
    ctx->add_warning(P265_WARNING_NONEXISTING_PPS_REFERENCED, false);
    return false;
  }

  seq_parameter_set_id = uvlc = get_uvlc(br);
  if (uvlc >= P265_MAX_SPS_SETS ||
      uvlc == UVLC_ERROR) {
    ctx->add_warning(P265_WARNING_NONEXISTING_SPS_REFERENCED, false);
    return false;
  }
  ...
  if (tiles_enabled_flag) {
	...
	num_tile_columns+1 > P265_MAX_TILE_COLUMNS) {
      ctx->add_warning(P265_WARNING_PPS_HEADER_INVALID, false);
      return false;
    }
    num_tile_columns++;
	...
	num_tile_rows+1 > P265_MAX_TILE_ROWS) {
      ctx->add_warning(P265_WARNING_PPS_HEADER_INVALID, false);
      return false;
    }
    num_tile_rows++;
  ...
  if (pic_scaling_list_data_present_flag) {
    P265_error err = read_scaling_list(br, sps.get(), &scaling_list, true);
    if (err != P265_OK) {
...
// {
//   if (pic_parameter_set_id >= P265_MAX_PPS_SETS) {
//     errqueue->add_warning(P265_WARNING_NONEXISTING_PPS_REFERENCED, false);
//     return false;
//   }
//   out.write_uvlc(pic_parameter_set_id);

//   if (seq_parameter_set_id >= P265_MAX_PPS_SETS) {
//     errqueue->add_warning(P265_WARNING_NONEXISTING_SPS_REFERENCED, false);
//     return false;
//   }
...
//   if (tiles_enabled_flag) {
//     if (num_tile_columns > P265_MAX_TILE_COLUMNS) {
//       errqueue->add_warning(P265_WARNING_PPS_HEADER_INVALID, false);
//       return false;
//     }
//     out.write_uvlc(num_tile_columns-1);

//     if (num_tile_rows > P265_MAX_TILE_ROWS) {
//       errqueue->add_warning(P265_WARNING_PPS_HEADER_INVALID, false);
//       return false;
//     }
...
//   if (pic_scaling_list_data_present_flag) {
//     P265_error err = write_scaling_list(out, sps, &scaling_list, true);
//     if (err != P265_OK) {
