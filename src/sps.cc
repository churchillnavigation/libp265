...
#define READ_VLC_OFFSET(variable, vlctype, offset)   \
  if ((vlc = get_ ## vlctype(br)) == UVLC_ERROR) {   \
    errqueue->add_warning(P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE, false);  \
    return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE; \
  } \
  variable = vlc + offset;
...
P265_error seq_parameter_set::read(error_queue* errqueue, bitreader* br)
{
  int vlc;
  ...
  if (seq_parameter_set_id >= P265_MAX_SPS_SETS) {
    return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE;
  }
  ...
  if (chroma_format_idc<0 ||
      chroma_format_idc>3) {
    errqueue->add_warning(P265_WARNING_INVALID_CHROMA_FORMAT, false);
    return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE;
  }
  ...
  if (bit_depth_luma > 16 ||
      bit_depth_chroma > 16) {
    errqueue->add_warning(P265_WARNING_SPS_HEADER_INVALID, false);
    return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE;
  }
  ...
  if (log2_min_luma_coding_block_size > 6) { return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE; }
  if (log2_min_luma_coding_block_size + log2_diff_max_min_luma_coding_block_size > 6) { return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE; }
  if (log2_min_transform_block_size > 5) { return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE; }
  if (log2_min_transform_block_size + log2_diff_max_min_transform_block_size > 5) { return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE; }
  ...
  if (scaling_list_enable_flag) {
    ...
    if (sps_scaling_list_data_present_flag) {
      ...
      P265_error err;
      if ((err=read_scaling_list(br,this, &scaling_list, false)) != P265_OK) {
        return err;
      }
    }
    else {
  ...
  if (num_short_term_ref_pic_sets < 0 ||
    ...
    errqueue->add_warning(P265_WARNING_NUMBER_OF_SHORT_TERM_REF_PIC_SETS_OUT_OF_RANGE, false);
    return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE;
  }
  ...
  for (int i = 0; i < num_short_term_ref_pic_sets; i++) {
    ...
    if (!success) {
      return P265_WARNING_SPS_HEADER_INVALID;
    }
  ...
  if (sps_range_extension_flag) {
    P265_error err = range_extension.read(errqueue, br);
    if (err != P265_OK) { return err; }
  }
  ...
  */
  ...
  P265_error err = compute_derived_values();
  if (err != P265_OK) { return err; }
...
P265_error seq_parameter_set::compute_derived_values(bool sanitize_values)
{
  ...
  if (Log2MaxTrafoSize > libP265_min(Log2CtbSizeY,5)) {
...
P265_error read_scaling_list(bitreader* br, const seq_parameter_set* sps,
...
// P265_error seq_parameter_set::write(error_queue* errqueue, CABAC_encoder& out)
// {
...
//       P265_error err;
//       if ((err=write_scaling_list(out,this, &scaling_list, false)) != P265_OK) {
//         return err;
//       }
//     }
//   }
...
P265_error sps_range_extension::read(error_queue* errqueue, bitreader* br)
{
