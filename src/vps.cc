...
P265_error video_parameter_set::read(error_queue* errqueue, bitreader* reader)
{
  int vlc;

  video_parameter_set_id = vlc = get_bits(reader, 4);
  if (vlc >= P265_MAX_VPS_SETS) return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE;

  skip_bits(reader, 2);
  vps_max_layers = vlc = get_bits(reader,6) +1;
  if (vlc > 63) return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE; // vps_max_layers_minus1 (range 0...63)

  vps_max_sub_layers = vlc = get_bits(reader,3) +1;
  if (vlc >= MAX_TEMPORAL_SUBLAYERS) return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE;
  ...
  if (vps_timing_info_present_flag) {
    ...
    if (vps_poc_proportional_to_timing_flag) {
      ...
      for (int i=0; i<vps_num_hrd_parameters; i++) {
        ...
        return P265_OK; // TODO: decode hrd_parameters()
      }
    }
  }
...
// P265_error video_parameter_set::write(error_queue* errqueue, CABAC_encoder& out) const
// {
//   if (video_parameter_set_id >= P265_MAX_VPS_SETS) return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE;
//   out.write_bits(video_parameter_set_id,4);

//   out.write_bits(0x3,2);
//   out.write_bits(vps_max_layers-1,6);

//   if (vps_max_sub_layers >= MAX_TEMPORAL_SUBLAYERS) return P265_ERROR_CODED_PARAMETER_OUT_OF_RANGE;
