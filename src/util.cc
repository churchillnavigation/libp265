...
LIBP265_API void P265_disable_logging() // DEPRECATED
{
  disable_logging_OLD=1;
}


LIBP265_API void P265_set_verbosity(int level)
{
  verbosity = level;
}

#if defined(P265_LOG_ERROR) || defined(P265_LOG_INFO) || defined(P265_LOG_DEBUG) || defined(P265_LOG_INFO)
...
static void (*debug_image_output_func)(const struct P265_image*, int slot) = NULL;

void debug_set_image_output(void (*func)(const struct P265_image*, int slot))
{
  debug_image_output_func = func;
}

void debug_show_image(const struct P265_image* img, int slot)
{
