...
#ifdef _MSC_VER
#define LIBP265_DECLARE_ALIGNED( var, n ) __declspec(align(n)) var
#define likely(x)      (x)
#define unlikely(x)    (x)
#else
#define LIBP265_DECLARE_ALIGNED( var, n ) var __attribute__((aligned(n)))
#define likely(x)      __builtin_expect(!!(x), 1)
#define unlikely(x)    __builtin_expect(!!(x), 0)
#endif

#if defined(__GNUC__) && (__GNUC__ >= 4)
#define LIBP265_CHECK_RESULT __attribute__ ((warn_unused_result))
#elif defined(_MSC_VER) && (_MSC_VER >= 1700)
#define LIBP265_CHECK_RESULT _Check_return_
#else
#define LIBP265_CHECK_RESULT
#endif
...
#define ALIGNED_32( var ) LIBP265_DECLARE_ALIGNED( var, 32 )
#define ALIGNED_16( var ) LIBP265_DECLARE_ALIGNED( var, 16 )
#define ALIGNED_8( var )  LIBP265_DECLARE_ALIGNED( var, 8 )
#define ALIGNED_4( var )  LIBP265_DECLARE_ALIGNED( var, 4 )
...
#define libP265_min(a,b) (((a)<(b)) ? (a) : (b))
#define libP265_max(a,b) (((a)>(b)) ? (a) : (b))

LIBP265_INLINE static int ceil_div(int num,int denom)
{
  num += denom-1;
  return num/denom;
}

LIBP265_INLINE static int ceil_log2(int val)
{
  int n=0;
  while (val > (1<<n)) {
    n++;
...
LIBP265_INLINE static int Log2(int v)
{
  int n=0;
  while (v>1) {
    n++;
...
LIBP265_INLINE static int Log2SizeToArea(int v)
{
  return (1<<(v<<1));
}
...
#if defined(P265_LOG_ERROR) || defined(P265_LOG_INFO) || defined(P265_LOG_DEBUG) || defined(P265_LOG_TRACE)
# define P265_LOGGING 1
